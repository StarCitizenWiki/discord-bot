...
const createEmbed = (data) => {
  ...
  for (const shop of shopData) {
    reply.addField(shop.name_raw, `${Math.round(shop.items.data[0].price_calculated).toString()} aUEC (${shop.items.data[0].buyable === true ? 'Kauf' : shop.items.data[0].sellable === true ? 'Verkauf' : ''})`, true);
  }
